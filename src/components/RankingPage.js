...
const RankingPage = ({ topTraits, setTopTraits, history, fetchTopTraits}) => {
    ...
    const buildMerge = (list) => {
        ...
        if(listLen > 6){ finishedList.current = finishedList.current.slice(listLen-7)}
        buildJoinStack(list);
    ...
    return (
            <Grid
                ...
                direction="column"
                >
             <Grid item sx={ isMobile && {paddingBottom: '10vh'}}>
                 <FadeTextSeries stringArray={['Select the trait you value more', 'This is a test of a long string that was not long enough to test text wrap', 'test 3', 'Lorem Ipsum Dolar']} variant={isMobile ? 'h3' : 'h5'} />
             </Grid>
                <Grid item>
               <Grid container
